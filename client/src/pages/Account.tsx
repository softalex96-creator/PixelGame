...
export default function Account() {
  ...
  const { authAvailable, signIn, signOut, status, user } = useAuth();
  ...
  return <main className="account-page"><div className="container account-layout">
    ...
    <section className="account-section auth-account-section"><div className="section-heading"><div><p className="eyebrow">{t.signInEyebrow}</p><h2>{t.signInTitle}</h2></div></div>
      <div className="auth-panel panel-shell">{user ? <div className="auth-signed-in"><span className="auth-provider-badge">{user.provider === "google" ? <Chrome size={18} /> : <ShieldCheck size={18} />}</span><div><strong>{t.signedInAs}</strong><p>{user.email ?? `${t.steamId} ${user.sub}`}</p><small><ShieldCheck size={13} /> {t.secureProvider}</small></div><button className="button button-outline" onClick={() => void signOut()} disabled={status === "loading"}><LogOut size={15} />{t.signOut}</button></div> : <><p>{t.signInDescription}</p><div className="auth-provider-grid"><button className="auth-provider-button google-provider" onClick={() => signIn("google")} disabled={!authAvailable || status === "loading"}>{status === "loading" ? <LoaderCircle className="auth-spinner" size={18} /> : <Chrome size={18} />}<span>{t.continueWithGoogle}</span></button><button className="auth-provider-button steam-provider" onClick={() => signIn("steam")} disabled={!authAvailable || status === "loading"}><span><ShieldCheck size={18} />{t.continueWithSteam}</span><small>{t.secureProvider}</small></button><button className="auth-provider-button" disabled title={t.providerPreparing}><span><Apple size={18} />{t.appleProvider}</span><small>{t.providerPreparing}</small></button><button className="auth-provider-button" disabled title={t.providerPreparing}><span><Send size={18} />{t.telegramProvider}</span><small>{t.providerPreparing}</small></button></div><small className="auth-security-note"><ShieldCheck size={13} />{t.secureProvider}</small></>}</div>
    </section>
