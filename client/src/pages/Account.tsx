...
export default function Account() {
  ...
  const savedProducts = products.filter((product) => favouriteIds.includes(product.id)).map((product) => getLocalizedProduct(product, locale));
  ...
  return <main className="account-page"><div className="container account-layout">
    <header className="account-hero"><p className="eyebrow">{t.accountEyebrow}</p><h1>{t.accountTitle}</h1><p>{t.accountDescription}</p>
      <div className="account-stats"><span><b>{orders.length}</b>{t.ordersCount}</span><span><b>{savedProducts.length}</b>{t.savedCount}</span></div>
    </header>
